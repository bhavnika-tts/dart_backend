...
Future<Response> onRequest(RequestContext context) async {
  ...
  try {
    ...
    final status = body['status']?.toString().trim() ?? 'closed';
    ...
    return Response.json(
      body: {'success': true, 'message': 'Support ticket status updated successfully'},
    );
  } catch (error) {
    return Response.json(
      statusCode: HttpStatus.internalServerError,
      body: {'message': 'Failed to update ticket status', 'error': error.toString()},
    );
  }
}
